...
select 'AUDIT ' || m.name ||
       decode(u.name, 'PUBLIC', ' ', ' BY ' || u.name) ||
       decode(nvl(a.success, 0) + (10 * nvl(a.failure, 0)),
              ...
              22,
              ' BY ACCESS',
              ' /* not possible */ ') || ' ;' "AUDIT STATEMENT"
  FROM sys.audit$ a, sys.user$ u, sys.stmt_audit_option_map m
 WHERE a.user# = u.user#
   AND a.option# = m.option#
   and bitand(m.property, 1) != 1
   and a.proxy# is null
   and a.user# <> 0
UNION
select 'AUDIT ' || m.name ||
       decode(u1.name, 'PUBLIC', ' ', ' BY ' || u1.name) ||
  ...
  FROM sys.audit$                a,
       sys.user$                 u1,
       sys.user$                 u2,
       sys.stmt_audit_option_map m
 WHERE a.user# = u2.user#
   AND a.option# = m.option#
   and a.proxy# = u1.user#
   and bitand(m.property, 1) != 1
   and a.proxy# is not null
UNION
select 'AUDIT ' || p.name ||
       decode(u.name, 'PUBLIC', ' ', ' BY ' || u.name) ||
       decode(nvl(a.success, 0) + (10 * nvl(a.failure, 0)),
...
select 'NOAUDIT ' || m.name ||
       ...
       decode(nvl(a.success, 0) + (10 * nvl(a.failure, 0)),
              ...
              22,
              ' ',
              ' /* not possible */ ') || ' ;' "NOAUDIT STATEMENT"
  FROM sys.audit$ a, sys.user$ u, sys.stmt_audit_option_map m
 WHERE a.user# = u.user#
   AND a.option# = m.option#
   and bitand(m.property, 1) != 1
...
select unique '-- Please correct the problem described in note 455565.1:' || chr(13) || chr(10) || 'delete from sys.audit$ where user#=0 and proxy# is null;' || chr(13) || chr(10) || 'commit;'
  from sys.audit$
 where user# = 0
   and proxy# is null;

select '-- Please correct the problem described in bug 6636804:' || chr(13) ||
       chr(10) || 'update sys.STMT_AUDIT_OPTION_MAP set option#=234' ||
...
select '-- Please correct the problem described in bug 6124447:' || chr(13) ||
       chr(10) || 'noaudit truncate;'
  from sys.audit$
 where option# = 155;

select unique '-- Please correct the problem described in note 1529792.1:' || chr(13) || chr(10) || 'insert into javaobj$ select object_id,' || chr(13) || chr(10) || '(select AUDIT$ from sys.javaobj$ where rownum=1)' || chr(13) || chr(10) || ' from dba_objects where object_type=''JAVA CLASS''' || chr(13) || chr(10) || ' and status=''VALID'' and object_id not in (select obj# from sys.javaobj$);' || chr(13) || chr(10) || 'commit;'
  from dba_objects
