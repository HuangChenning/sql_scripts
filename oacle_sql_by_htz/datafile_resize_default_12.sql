...
DECLARE
   ...
   v_size         NUMBER;
BEGIN
   for c_con in (select distinct con_id from cdb_tablespaces) loop
   FOR c_tb IN (SELECT tablespace_name, a.contents
                 ...
                 WHERE    (a.tablespace_name IN ('SYSTEM',
                                                ...
                                                'TEMP',
                                                'USERS')
                       OR a.contents = 'UNDO') and a.con_id=c_con.con_id)
   LOOP
      ...
      THEN
         SELECT SUM (bytes)
          ...
          WHERE a.tablespace_name = c_tb.tablespace_name and a.con_id=c_con.con_id;
