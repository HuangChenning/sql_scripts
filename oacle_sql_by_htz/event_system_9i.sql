...
col days_old for 99999 heading 'Hours|Start Up'
PROMPT
PROMPT +------------------------------------------------------------------------+
PROMPT | display system event not include idle event                            |
PROMPT +------------------------------------------------------------------------+ 
PROMPT
/* Formatted on 2012/12/24 9:51:20 (QP5 v5.215.12089.38647) */
  SELECT substr(a.event,1,40) event,
         a.total_waits,
         round(a.time_waited/100,3) time_wait,
         round((a.time_waited/c.total_time)*100,2) time_100,
         ROUND (average_wait/100, 3) ave_wait,
         ROUND ((SYSDATE - b.startup_time)*24,2 ) days_old
