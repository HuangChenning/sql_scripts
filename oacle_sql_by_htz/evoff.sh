:
# evoff.sh - Disables events on processes for a given Oracle instance id
#
...
echo "Events were set OFF for the following Oracle processes :" >> $TMPFILE

for proc in `ps -ef | grep "$STEM" | awk '/grep/ {next} \
{print $2}'`
do
 echo "Disabling events for process $proc"
 echo "$proc" >> $TMPFILE
 CNT=`echo "$CNT+1"|bc`
 svrmgrl <<!
connect internal
oradebug setospid $proc 
oradebug event 10235 trace name context off
!
done
