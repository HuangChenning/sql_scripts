...
col o_o for a25 heading 'OBJECT|OWNER:NAME'
SELECT SUBSTR(b.event, 1, 19) event,
       ...
       d.owner || ':' || d.object_name o_o,
       b.username,
       SUBSTR(b.osuser || '@' || b.machine || '@' || b.process, 1, 30) client
  FROM v$session b, v$process c, v$session_wait s, dba_objects d
 WHERE b.paddr = c.addr
   AND s.SID = b.SID
   AND b.event like 'enq%SQ-contention' or b.event like 'enq%SV-contention'
   and b.P2 = d.object_id
/
@sqlplusset