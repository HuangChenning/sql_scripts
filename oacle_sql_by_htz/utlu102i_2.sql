...
Fetched rdbms/admin/utlu102i.sql@@/st_rdbms_10.2/LATEST to /ade/ewittenb_10205/rdbms/admin/utlu102i.sql#st_rdbms_10.2_LATEST
its affiliates. 
...
Rem    bmccarth    04/08/10 - parsing issue 9559513
Rem                         - cursor_space_for_time
Rem    cdilling    01/18/10 - calibrate tablespaces sizes for 10.2.0.5
Rem    cmlim       12/15/09 - bug 9174373 - flag sga_target if lower Oracle is
Rem                           102 and user had set it to less than minimum value
Rem    cdilling    11/25/09 - add more invalid object exceptions
Rem    cdilling    10/08/09 - include invalid objects
Rem    mdietric    05/26/08 - Removing large_pool_size check - bug7015534
Rem    yuli        04/20/08 - bump to 10.2.0.5
Rem    cdilling    02/12/08 - increase shared pool size to 192 MB
Rem    cdilling    09/26/07 - add more invalid objects for patch upgrade
Rem    yuli        07/25/07 - bump to 10.2.0.4
Rem    dkapoor     05/24/07 - add OCM warning
Rem    rburns      02/14/07 - change APEX warning
Rem    cdilling    04/18/07 - Backport cdilling_bug-5731352
Rem    cdilling    02/27/07 - Backport cdilling_bug-5715271
Rem    cdilling    02/27/07 - Backport cdilling_bug-4905742 from main
Rem    rburns      12/21/06 - fix undo parameters
Rem    rburns      10/31/06 - remove unnecessary warnings for patch upgrades
Rem                         - calibrate tablespace estimates for 10.2.0.3
Rem    rburns      06/24/06 - check for XE and APEX scripts 
Rem    cdilling    06/20/06 - Backport rburns_bug-4565939 from main 
Rem    yuli        06/16/06 - bump to 10.2.0.3 
Rem    cdilling    04/04/06 - Backport cdilling_bug-4864648 from main 
Rem    cdilling    03/27/06 - Backport rburns_bug-4429903 from main 
Rem    cdilling    03/27/06 - Backport rburns_bug-4466438 from main 
Rem    cdilling    01/24/06 - add dict managed temp tablespace for bug 4864648 
Rem    rburns      10/07/05 - check UNDO tablespaces 
Rem    rburns      10/25/05 - correct patch tablespace estimates 
Rem    rburns      10/09/05 - Add EM space for DBUA 
Rem    rburns      10/08/05 - add 16k blocksize estimates 
Rem    rburns      09/22/05 - add 10.2 patch release 
Rem    rburns      08/23/05 - change stale statistics check 
Rem    yuli        08/19/05 - bump to 10.2.0.2 
Rem    rburns      06/27/05 - update deprecated params 
...
DECLARE
  ...
  utlu_version CONSTANT VARCHAR2(30) := '10.2.0.5';
...
-- *****************************************************************
  ...
  db_undo         VARCHAR2(30):=NULL;
  db_undo_tbs     VARCHAR2(30):=NULL;
...
-- index values for components (order as in upgrade script)
  ...
  htmldb  CONSTANT BINARY_INTEGER:=22;
  stats   CONSTANT BINARY_INTEGER:=23;

  max_comps CONSTANT BINARY_INTEGER :=23; -- include STATS for space calcs
...
-- *****************************************************************
   ...
   xe_patch          BOOLEAN := FALSE; -- TRUE when XE database being patched
   apx_needed        BOOLEAN := FALSE; -- TRUE when apxxemig script needed
...
ELSE
    ...
    IF xe_patch THEN
       DBMS_OUTPUT.PUT_LINE(
           '... Application Express will be upgraded to version 2.2');
       DBMS_OUTPUT.PUT_LINE(
           '... during the patch upgrade of this XE database');
       DBMS_OUTPUT.PUT_LINE ('.');
    END IF;
END IF;
...
BEGIN
   ...
   IF display_xml THEN
      ...
      IF apx_needed THEN
          DBMS_OUTPUT.PUT_LINE('<warning name="APEX_NEEDED"/>');
      END IF;
   ...
   ELSE
      IF version_mismatch or cluster_dbs OR dip_user_exists OR 
         ...
         invalid_objs OR cdc_data or ocm_user_exists or post_upgrade or apx_needed THEN
            display_banner;
           ...
           IF cmp_info(i).processed and NOT cmp_info(i).install THEN
             IF cmp_info(i).schema IS NOT NULL THEN
                EXECUTE IMMEDIATE '
                   ...
                   begin
                      p_count:=0;
                      dbms_stats.gather_schema_stats( :1, options=>''list auto'', objlist=>p_otab);
                ...
                USING '"' || cmp_info(i).schema || '"', OUT p_count;
                IF (p_count > 0) THEN
      ...
      IF apx_needed THEN
         DBMS_OUTPUT.PUT_LINE(
         'WARNING: --> Application Express requires upgrading. Download');
         DBMS_OUTPUT.PUT_LINE(
         '.... APEX version 2.2 from OTN before upgrading the XE database.');
      END IF;
...
-- *****************************************************************
   ...
   -- Check for XE database and APEX patch script
   BEGIN
      EXECUTE IMMEDIATE
      ...
      IF p_edition = 'XE' THEN 
         xe_patch := TRUE;
         SELECT name INTO p_schema FROM user$ WHERE name='FLOWS_020100';
         -- APEX schema exists, so look for patch script 
         apx_needed := TRUE;
      END IF; -- XE edition
   EXCEPTION
      WHEN OTHERS THEN NULL;  -- no edition column or no FLOWS schema
   END;      
...
--   store_removed(idx,'mts_sessions');
   ...
   store_removed(idx,'cursor_space_for_time');
...
-- Initialize parameters with minimum values
   ...
   idx := 0;
   store_minvalue(idx,'sga_target',      312*1024*1024); --  312 MB 
   tg_idx := idx;
   store_minvalue(idx,'shared_pool_size',192*1024*1024); -- 192 MB
   sp_idx := idx;  -- save the value for isga calculation
   store_minvalue(idx,'java_pool_size',   64*1024*1024); -- 64 MB
   jv_idx := idx;
   IF dbv != 102 THEN -- streams pool not needed for patch upgrade
   ...
   pg_idx := idx;
   IF dbv != 102 THEN  -- max files not need for patch upgrade
...
-- Initialize script names
 IF dbv = 102 THEN
   ...
   cmp_info(odm).script     := '?/rdbms/admin/odmpatch.sql';
   ...
   cmp_info(htmldb).script  := '?/htmldb/core/htmlpatch.sql';
 ELSE
   cmp_info(catalog).script := '?/rdbms/admin/catalog.sql';
   cmp_info(catproc).script := '?/rdbms/admin/catproc.sql';
...
-- kbytes for component installs (into SYSTEM and DEFAULT tablespaces)
   ...
   IF db_block_size = 16384 THEN
      cmp_info(xdb).ins_def_kbytes:=     97536;
   ELSE
      cmp_info(xdb).ins_def_kbytes:=     49408;
   END IF;
   cmp_info(em).ins_sys_kbytes:=      22528;
   cmp_info(em).ins_def_kbytes:=      51200;
   cmp_info(ordim).ins_sys_kbytes:=   23064;
   cmp_info(ordim).ins_def_kbytes:=     448;

   IF dbv = 817 THEN

      cmp_info(catalog).sys_kbytes:= 146288;
      cmp_info(catproc).sys_kbytes:=    384;
      cmp_info(javavm).sys_kbytes:=   43776;
      cmp_info(xml).sys_kbytes:=       3456;
      cmp_info(catjava).sys_kbytes:=   4096;
      cmp_info(context).sys_kbytes:=   3776;
      cmp_info(ordim).sys_kbytes:=    15104;
      cmp_info(sdo).sys_kbytes:=       1344;
      cmp_info(ols).sys_kbytes:=        384;
      cmp_info(stats).sys_kbytes:=     7360;

      cmp_info(catproc).sysaux_kbytes:=  69248;
      cmp_info(ordim).sysaux_kbytes:=      256;
      cmp_info(sdo).sysaux_kbytes:=        320;
      cmp_info(stats).sysaux_kbytes:= 1760+2534;
      ...
      cmp_info(sdo).def_ts_kbytes:=      26496; -- MDSYS 

   ELSIF dbv=901 THEN

      cmp_info(catalog).sys_kbytes:=   1600;
      cmp_info(catproc).sys_kbytes:= 111792; 
      cmp_info(javavm).sys_kbytes:=   40592;
      cmp_info(xml).sys_kbytes:=       1648;
      cmp_info(catjava).sys_kbytes:=   2240;
      cmp_info(context).sys_kbytes:=   1960;
      cmp_info(owm).sys_kbytes:=       2096;
      cmp_info(amd).sys_kbytes:=       3536;
      cmp_info(ordim).sys_kbytes:=     8400;
      cmp_info(sdo).sys_kbytes:=      12960;
      cmp_info(wk).sys_kbytes:=        1648;
      cmp_info(ols).sys_kbytes:=        264;
      cmp_info(stats).sys_kbytes:=     7888;

      cmp_info(catalog).sysaux_kbytes:=  15347;
      cmp_info(catproc).sysaux_kbytes:=  76173;
      cmp_info(owm).sysaux_kbytes:=      12250;
      ...
      cmp_info(owm).def_ts_kbytes:=        928; -- WMSYS
      cmp_info(amd).def_ts_kbytes:=       9536; -- ORAOLAP
      cmp_info(wk).def_ts_kbytes:=  6195+15770; -- WKSYS + WK_TEST
      cmp_info(ols).def_ts_kbytes:=        224; -- LBACSYS
      cmp_info(ordim).def_ts_kbytes:=      192; -- ORDSYS
      cmp_info(sdo).def_ts_kbytes:=      24922; -- MDSYS
      cmp_info(context).def_ts_kbytes:=    563; -- CTXSYS

   ELSIF dbv=920 THEN

      cmp_info(catalog).sys_kbytes:=   1664;  
      cmp_info(catproc).sys_kbytes:=  482752-384320;  
      cmp_info(javavm).sys_kbytes:=     768;
      cmp_info(xml).sys_kbytes:=         64;
      cmp_info(catjava).sys_kbytes:=     64;
      cmp_info(context).sys_kbytes:=   1408; 
      cmp_info(xdb).sys_kbytes:=       5824;
      cmp_info(owm).sys_kbytes:=       2112;
      cmp_info(odm).sys_kbytes:=       1536;
      cmp_info(mgw).sys_kbytes:=       1792;
      cmp_info(aps).sys_kbytes:=          0;
      cmp_info(amd).sys_kbytes:=       1024;
      cmp_info(xoq).sys_kbytes:=       2240;
      cmp_info(ordim).sys_kbytes:=     7104;
      cmp_info(sdo).sys_kbytes:=      12864;
      cmp_info(wk).sys_kbytes:=         256;
      cmp_info(ols).sys_kbytes:=         64;
      cmp_info(stats).sys_kbytes:=     6976;

      cmp_info(catalog).sysaux_kbytes:=  13952; 
      cmp_info(catproc).sysaux_kbytes:=  69248;
      cmp_info(owm).sysaux_kbytes:=      11136 ;
      cmp_info(odm).sysaux_kbytes:=        512;
      cmp_info(aps).sysaux_kbytes:=      24832;
      cmp_info(xoq).sysaux_kbytes:=       1408;
      cmp_info(sdo).sysaux_kbytes:=         64;
      cmp_info(stats).sysaux_kbytes:= 1600+2304;

      cmp_info(amd).def_ts_kbytes:=       1152; -- OLAPSYS
      cmp_info(context).def_ts_kbytes:=    512; -- CTXSYS
      cmp_info(wk).def_ts_kbytes:=  5632+14336; -- WKSYS + WK_TEST
      cmp_info(odm).def_ts_kbytes:=        256; -- ODM + ODM_MTR
      cmp_info(owm).def_ts_kbytes:=       6400; -- WMSYS
      cmp_info(ordim).def_ts_kbytes:=      384; -- ORDSYS
      cmp_info(sdo).def_ts_kbytes:=      22656; -- MDSYS
      cmp_info(catproc).def_ts_kbytes:=   7040; -- SYSTEM user
      cmp_info(xdb).def_ts_kbytes:=       3968; -- XDB
      cmp_info(ols).def_ts_kbytes:=        448; -- LBACSYS

   ELSIF dbv = 101 THEN

      cmp_info(catalog).sys_kbytes:=   1088;
      cmp_info(catproc).sys_kbytes:=  32896; 
      cmp_info(javavm).sys_kbytes:=   11328;  
      cmp_info(xml).sys_kbytes:=       8320;  
      cmp_info(catjava).sys_kbytes:=     64;
      cmp_info(context).sys_kbytes:=    192;  
      cmp_info(xdb).sys_kbytes:=       2624;  
      cmp_info(owm).sys_kbytes:=       2112;  
      cmp_info(odm).sys_kbytes:=       9280;  
      cmp_info(mgw).sys_kbytes:=       2112;
      cmp_info(aps).sys_kbytes:=          0;
      cmp_info(amd).sys_kbytes:=       1088;
      cmp_info(xoq).sys_kbytes:=        448;
      cmp_info(ordim).sys_kbytes:=     3776;
      cmp_info(sdo).sys_kbytes:=       4352;
      cmp_info(wk).sys_kbytes:=        2048;
      cmp_info(ols).sys_kbytes:=       1024;
      cmp_info(exf).sys_kbytes:=       1024;
      cmp_info(em).sys_kbytes:=        2240;
      cmp_info(stats).sys_kbytes:= 1856+2304;

      cmp_info(catalog).sysaux_kbytes:=      0;
      cmp_info(catproc).sysaux_kbytes:= 106688;  
      cmp_info(owm).sysaux_kbytes:=          0;
      cmp_info(xdb).sysaux_kbytes:=        768;
      cmp_info(odm).sysaux_kbytes:=          0;
      cmp_info(aps).sysaux_kbytes:=       3776;
      cmp_info(xoq).sysaux_kbytes:=       4992;
      cmp_info(ordim).sysaux_kbytes:=      128;
      cmp_info(sdo).sysaux_kbytes:=          0;
      cmp_info(exf).sysaux_kbytes:=          0;
      cmp_info(em).sysaux_kbytes:=           0;
      cmp_info(stats).sysaux_kbytes:= 2944+1664;

      cmp_info(amd).def_ts_kbytes:=        128; -- OLAPSYS
      cmp_info(context).def_ts_kbytes:=     64; -- CTXSYS
      cmp_info(wk).def_ts_kbytes:=        1792; -- WKSYS + WK_TEST
      cmp_info(odm).def_ts_kbytes:=        128; -- DMSYS
      cmp_info(owm).def_ts_kbytes:=        768; -- WKSYS
      cmp_info(ordim).def_ts_kbytes:=        0; -- ORDSYS
      cmp_info(sdo).def_ts_kbytes:=      25408; -- MDSYS
      cmp_info(catproc).def_ts_kbytes:=   2048; -- SYSTEM 
      cmp_info(xdb).def_ts_kbytes:=       3264; -- XDB
      cmp_info(ols).def_ts_kbytes:=        256; -- LBACSYS
      cmp_info(exf).def_ts_kbytes:=       1024; -- EXFSYS
      cmp_info(em).def_ts_kbytes:=        1536; -- SYSMAN

   ELSIF dbv = 102 THEN

      cmp_info(catproc).sys_kbytes:=      4096;  
      cmp_info(javavm).sys_kbytes:=        512;  
      cmp_info(odm).sys_kbytes:=           256;  
      cmp_info(context).sys_kbytes:=       128;  
      cmp_info(xdb).sys_kbytes:=           256;  
      cmp_info(amd).sys_kbytes:=           128;  
      cmp_info(xoq).sys_kbytes:=          1280;  
      cmp_info(ordim).sys_kbytes:=        3072;  
      cmp_info(sdo).sys_kbytes:=          2048;  
      cmp_info(wk).sys_kbytes:=            128;  
      cmp_info(ols).sys_kbytes:=           128;  
      cmp_info(em).sys_kbytes:=           2048;  

      cmp_info(catproc).sysaux_kbytes:=   6144;  
      cmp_info(aps).sysaux_kbytes:=       6144;  
      cmp_info(xoq).sysaux_kbytes:=       1024;  

      cmp_info(context).def_ts_kbytes:=    128;  
      cmp_info(sdo).def_ts_kbytes:=        256;  
      cmp_info(sdo).def_ts_kbytes:=        128;  
      cmp_info(em).def_ts_kbytes:=         512;  
      cmp_info(xdb).def_ts_kbytes:=        256;  
...
/* no HTML DB yet
 ...
 IF dbv != 102 THEN -- install required components on major release only
...
/* avoid CPU dependency in DIAG mode (cpu * 2097152) +  */
   ...
   IF mp(tg_idx).oldvalue != 0 THEN  -- SGA_TARGET in use
      ...
      END IF;
      FOR i IN 1..max_mp LOOP
        IF i NOT IN (tg_idx,cs_idx,jv_idx,sp_idx,st_idx) AND 
...
-- *****************************************************************
   ...
   FOR ts IN (SELECT tablespace_name, contents, extent_management 
       ...
       IF ts.tablespace_name IN ('SYSTEM', 'SYSAUX', db_undo_tbs) OR 
          ...
          IF ts_info(idx).temporary AND dbv != 817 AND
...
-- *****************************************************************
   ...
   BEGIN
      IF dbv NOT IN (817, 901, 102) THEN
         EXECUTE IMMEDIATE 
   ...
   IF cmp_info(owm).processed AND dbv IN (901, 920, 101) THEN
      BEGIN
  ...
  -- Check for INVALID objects
   ...
   -- Check for OLS
   IF cmp_info(ols).processed AND dbv != 102 THEN
      post_upgrade := TRUE;
   END IF;
   ...
   IF dbv = 817 OR stale_stats THEN
...
-- *****************************************************************
   ...
   FOR t IN 1..max_ts LOOP
       ...
       IF ts_info(t).name = db_undo_tbs AND
          ts_info(t).alloc < (351*1024) THEN
             delta_kbytes := ((351*1024) - ts_info(t).inuse);
       END IF;
        ...
        IF ts_info(t).addl > 0 OR ts_info(t).inc_by > 0 THEN
           ...
           IF ts_info(t).temporary AND dbv != 817 AND
...
-- *****************************************************************
   ...
   ELSE
      ...
      display_database;
      IF dbv NOT IN (101, 102) THEN  -- database not upgraded yet
         display_logfiles;
      END IF;
      display_tablespaces;
      display_rollback_segs;
