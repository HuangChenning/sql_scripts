...
#
# Adjustment: -O option was dropped from svmon parameters
#             to allow older svmon to work
#
# Usage: Run ora_mem_bc.pl -h for usage info
#
# Prerequisites:
...
sub SigHandler
{
  ...
  die "Thank you for using ora_mem_bc.pl ;-)";
}
...
#-----------------------------------------------------------------------
sub CheckOsCompatibility
{
  die "Sorry, this tool only supports AIX" if 'aix' ne $^O;

  my $szAixLevel = `oslevel`;
  $szAixLevel =~ /^(\d+\.\d+)\..*/;
  die "Unrecognized AIX version" if ! defined $1;

  my $szAixVer = $1;
  die "Tool only supports AIX 5.3 and 6.1" if $szAixVer !~ /5\.3|6\.1/;
}
...
{
  die <<EOM ;
usage: ora_mem_bc.pl <options| -h> [<wait seconds> [<count>] ]
...
# This function was adjusted to allow older svmon to work
#-----------------------------------------------------------------------
sub GetSvmonData {
  ...
  # ADJ my $szCmd = 'svmon -O segment=category -O filterprop=data -P ' . join " ", (sort keys %$rhProc);
  my $szCmd = 'svmon -P ' . join " ", (sort keys %$rhProc);
  my $szCmdOut = `$szCmd`;
...
#    } 
    ...
    # This is the start of memory area
    if($szLine =~ /Vsid\s+Esid\s+Type\s+Description/i) {
      $szArea = 'memory';
      next;
    }
    ...
    if($szArea eq 'memory') {
# ADJ  my $rRegexes = ('exclusive' eq $szArea) ? $rExclusiveRegexes : $rSharedRegexes;
#      next if ! defined $rRegexes;
      if(($szLine =~ m/$rExclusiveRegexes/) or ('N' eq $bSkipShared and $szLine =~ m/$rSharedRegexes/) or 
        ($szLine =~ m/$rUnrecognizedRegexes/ and $szLine !~ m/$rSkipRegexes/)) {
        ...
        if(defined($szKey) && 0 != length($szKey)) {
          ...
          } elsif(('N' eq $bSkipShared) and ($szLine =~ m/$rSharedRegexes/)) {
