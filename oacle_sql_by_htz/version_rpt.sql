 ...
 */
 ...
 PIPE ROW('Version Count Report Version 3.2.1 -- Today''s Date '||to_char(sysdate,'dd-mon-yy hh24:mi')) ;
 PIPE ROW('RDBMS Version :'||v_version||' '||v_instance);
    ...
    */   
     ...
     v_query:= replace(v_query,'sql_text',q'"Pseudo_cursor||'(PseudoCursor of '||owner||'.'||object_name||')'"');
     debugme(v_query); 
    ...
    debugme('Header');
     ...
     v_all_no:=-1;
         ...
         debugme('Build Where');
          ...
          if v_version like '9%' or v_sql_id is  null then
           v_sql_where:=' WHERE '||v_coladdr||'=HEXTORAW('''||V_ADDR||''')';
          else
         ...
         debugme(substr(v_sql_where||v_sql_where2,-80));
...
--            debugme('Decode row '||v_count||' column '||i);
         ...
         if v_obs>0 then
          ...
          V_NO:=NULL;
          v_query:=q'"select max(SYSTEM_VALUE) into :v_no from h$parameter where name='_cursor_obsolete_threshold' "';
