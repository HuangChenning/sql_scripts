...
SELECT 'check_'||instance_NAME||'_'||host_name||'_'||to_char(sysdate,'yyyymmdd')||'.html' filename FROM v$instance;
spool &filename
...
SELECT /*+no_merge(A),no_merge(B)*/
 FILE_COUNT,
 ROUND(FILE_SIZE, 2) "FILE_SIZE(GB)",
 ROUND(FILE_SIZE - FREE, 2) "USED(GB)"
  FROM (SELECT SUM(FILE_COUNT) FILE_COUNT,
	       SUM(FILE_SIZE) / 1024 / 1024 / 1024 "FILE_SIZE"
	  FROM (SELECT COUNT(*) FILE_COUNT, SUM(BYTES) FILE_SIZE
		  FROM V$DATAFILE
		UNION ALL
		SELECT COUNT(*), SUM(BYTES) FROM V$TEMPFILE)) A,
       (SELECT SUM(BYTES) / 1024 / 1024 / 1024 FREE FROM DBA_FREE_SPACE);
prompt ****************************************
...
SELECT D.TABLESPACE_NAME "Name",
       ...
       D.logging,
       D.force_logging,
       D.extent_management||'['||D.allocation_type||']' extent_management,
...
SELECT INDEX_OWNER || '.' || INDEX_NAME INDEX_NAME,
  ...
  FROM DBA_IND_SUBPARTITIONS
   WHERE STATUS <> 'USABLE' ;
prompt ****************************************
prompt [TITLE] Partition Table with noPartition Index
prompt **************************************** 
...
SELECT OWNER, TABLE_NAME, LAST_ANALYZED
  FROM DBA_TABLES
 WHERE (LAST_ANALYZED IS NULL OR LAST_ANALYZED < SYSDATE - 180)
   AND OWNER NOT IN
...
prompt [TITLE] Privs
prompt **************************************** 
break on username on default_tablespace on temporary_tablespace
select 	username, 
	default_tablespace, 
...
prompt ****************************************  
select 'FILE_ID='||FILE_ID||' 30天内未备份' FROM (
SELECT FILE_ID FROM DBA_DATA_FILES
MINUS
SELECT DISTINCT FILE# FROM V$BACKUP_DATAFILE WHERE COMPLETION_TIME > SYSDATE - 30);
