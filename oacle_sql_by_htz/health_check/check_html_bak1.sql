...
SELECT 'Instance Name',Instance_Name FROM gv$instance
UNION ALL
SELECT 'Server Name',VALUE FROM v$parameter WHERE NAME = 'service_names'
UNION ALL
SELECT 'Usage','OLTP' FROM dual
UNION ALL
SELECT 'RDBMS Version/Release',banner  FROM v$version WHERE banner LIKE 'Oracle Database%'
UNION ALL
SELECT 'SQL*Net Version/Release',banner  FROM v$version WHERE banner LIKE 'NLSRTL%'
UNION ALL
SELECT '数据库目前总空间',SUM(BYTES)/1024/1024||' MB' FROM V$DATAFILE
UNION ALL
SELECT '数据库目前总使用量',SUM(BYTES) / 1024 / 1024 ||' MB' FROM DBA_FREE_SPACE
UNION ALL
...
------ [TITLE] HOST
--------------------------------------------------- 
-- prompt <a name="HOST"></a>
-- prompt <font size="+1" face="Arial,Helvetica,Geneva,sans-serif" color="#336699"><b>host check</b></font><hr align="left" width="460">
-- -- 空间占用
-- host df -m
-- host bdf
-- -- 内存，CPU，分页
