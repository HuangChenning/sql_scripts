REM srdc_xdbusage92check.sql - collect Oracle XDB Usage information 
define SRDCNAME='XDB_USAGE_CHECK_92'
...
cursor c_api is select owner,name,type from dba_dependencies
 where referenced_name in (
    ...
    where object_name like 'DBMS_XML%'
    or object_name like 'DBMS_XSL%')
  and TYPE !='SYNONYM'
  and owner !='SYS';
...
cursor c_res is select distinct (a.username) "USER",count (r.xmldata) "TOTAL"
 ...
 where sys_op_rawtonum (extractvalue (value(r),
                       '/Resource/OwnerID/text()')) =a.USER_ID
 group by a.username;
-- check xdbconfig.xml values
cursor c_config is select value(x).GETROOTELEMENT() NODENAME,
                   extractValue(value(x),'/*') NODEVALUE
                   from table(xmlsequence(extract(xdburitype(
                             '/xdbconfig.xml').getXML(),'//*[text()]'))) x;
...
DBMS_OUTPUT.PUT_LINE('Minimum version is 9.2.0.3.0. actual version is: '||
                     dbms_registry.version('XDB'));
end if;
if v_ver.version like '9.%' then DBMS_OUTPUT.PUT_LINE(' Doing  '||
...
DBMS_OUTPUT.PUT_LINE(v_config.NODENAME||'= = = '
                                      ||v_config.NODEVALUE);
else DBMS_OUTPUT.PUT_LINE('No Data Found');
end if;
end loop;
...
DBMS_OUTPUT.PUT_LINE(v_xml_tab_cols.owner||' has '
                                         ||v_xml_tab_cols.TOTAL
...
DBMS_OUTPUT.PUT_LINE(v_api.type||' '
                               ||v_api.owner
                               ||'.'
                               ||v_api.name);
else DBMS_OUTPUT.PUT_LINE('No Data Found');
...
DBMS_OUTPUT.PUT_LINE(v_res.USER||' has '
                               ||v_res.TOTAL||' resources.');
else DBMS_OUTPUT.PUT_LINE('No Data Found');
end if;
end loop;
close c_res;
...
EXCEPTION
     ...
     WHEN others THEN
       ...
       DBMS_OUTPUT.PUT_LINE('ERROR: '||v_errcode
                                     ||': '
                                     ||v_errmsg);
end;
/
