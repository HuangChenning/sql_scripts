...
PROMPT +------------------------------------------------------------------------+ 
PROMPT
SELECT TO_CHAR (a.begin_interval_time, 'yyyy-mm-dd hh24:mi:ss') startup_time, a.snap_id
  FROM sys.DBA_HIST_SNAPSHOT a, sys.v_$instance b
 WHERE a.startup_time > b.startup_time order by snap_id
/
PROMPT
PROMPT +------------------------------------------------------------------------+
...
SELECT a.sql_id,
         ...
         b.object_owner,
         b.object_name,
         round((c.BLOCKS*8)/1024) size
    FROM sys.dba_hist_sqlstat a, sys.DBA_HIST_SQL_PLAN b,dba_tables c
...
GROUP BY a.sql_id, a.plan_hash_value, b.object_owner,b.object_name,round((c.BLOCKS*8)/1024)
ORDER BY 3 
/
