Rem
Rem $Header: rdbms/admin/utlu112i.sql st_server_bmccarth_metalink_update2/1 2010/05/06 11:07:40 bmccarth Exp $
Rem
Rem utlu112i.sql
Rem
...
Rem
Rem      Supported releases: 9.2.0, 10.1.0, 10.2.0 and 11.1.0 and 11.2.0.1
Rem
Rem    NOTES
Rem      Run connected AS SYSDBA to the database to be upgraded
Rem
Rem    MODIFIED   (MM/DD/YY)
Rem    bmccarth    10/20/10 - My Oracle Support update:
Rem    cmlim       10/20/10 - bug 10210205: add time zone detection up to tz
Rem                           v14 for 9i databases
Rem    cmlim       08/28/10 - bug 10068587 - insert invalid objects into
Rem                           registry$sys_inv_objs/registry$nonsys_inv_objs
Rem                           if tbl already exists
Rem    vmedam      10/06/10 - bug 10175219
Rem    bmccarth    08/16/10 - bug 10017332 - obsolete
Rem                           cell_partition_large_extents
Rem    cmlim       07/08/10 - bug 9867959 - OLS & DV msgs to reflect 112
Rem                           installer
Rem    cmlim       07/06/10 - bug 9858126 - update time zone file detection for
Rem                           9i dbs (tz v14 should be last DST version for 9i)
...
Rem
Rem    bmccarth    04/26/10 - My Oracle Support upgrade:
Rem                         - Recommendation section
Rem                         - Remove stale stats check
Rem                         - linesize to 100
Rem    bmccarth    04/26/10 - My Oracle Support upgrade
...
-- *****************************************************************
  ...
  utlu_version     CONSTANT VARCHAR2(30) := '11.2.0.1';
  utlu_patchset    CONSTANT VARCHAR2(3)  := '.0';
  utlu_buildrev    CONSTANT VARCHAR2(3)  := '004';
  utlu_tz_version  CONSTANT NUMBER := 11;  -- Match with catupstr.sql
-- *****************************************************************
...
BEGIN
   EXECUTE IMMEDIATE 'SELECT NULL FROM sys.dba_tables t
      WHERE EXISTS 
         (SELECT 1 FROM sys.dba_queues q WHERE q.queue_table = t.table_name)
...
BEGIN
   ...
   ELSE
     ...
     IF qos_user_exists THEN
      ...
      -- bug 7193417: support timezone change in 11.2
...
BEGIN
   ...
   IF display_xml THEN
     -- Do nothing for XML output, the DBUA takes care of this
     RETURN;
   END IF;
...
-- *****************************************************************
  ...
  -- determine if memory_target value is set
   ...
   --
   -- Update registry$ with timezone information
   --
...
-- add 10% for 11g 
   cmp_info(javavm).ins_sys_kbytes:= 105972*1.2;
   cmp_info(xml).ins_sys_kbytes:=      4818*1.2;
   cmp_info(catjava).ins_sys_kbytes:=  5760*1.2;
   cmp_info(xdb).ins_sys_kbytes:=      8388*1.2;
   IF db_block_size = 16384 THEN
      cmp_info(xdb).ins_def_kbytes:= 110676*1.2;
   ELSE
      cmp_info(xdb).ins_def_kbytes:=  56064*1.2;
   END IF;
   cmp_info(ordim).ins_sys_kbytes:=   23064*1.2;
   cmp_info(ordim).ins_def_kbytes:=     448*1.2;
   ...
   IF dbv=920 THEN
      ...
      cmp_info(apex).def_ts_kbytes:=    184320; -- APEX
    ...
    ELSIF dbv = 101 THEN
      ...
      cmp_info(apex).def_ts_kbytes:=    184320; -- FLOWS
   ...
   ELSIF dbv = 102 THEN
      ...
      cmp_info(apex).def_ts_kbytes:=    184320; -- FLOWS_
...
-- *****************************************************************
   ...
   IF NOT db_readonly THEN
     BEGIN
       BEGIN
           -- Truncate the table first, just catch the 942 error and ignore
   ...
   -- Check for use of WITH TIME ZONE datatypes with pre-11g
   -- version of the time zone region file
...
-- *****************************************************************
-- END of Collect for RECOMMENDATION Section
-- *****************************************************************
        ...
        FOR i IN 1..max_comps LOOP 
           IF ts_info(t).name = cmp_info(i).def_ts AND
              ...
              ELSE  -- use default tablespace amount
                 ...
                 -- note: the size needed for apex for upgrades (whether
                 -- apex is in sysaux or separate tblspace) would be added here
