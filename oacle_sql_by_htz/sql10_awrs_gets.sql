...
set term off
set pagesize 0
PROMPT  'DB BEGINE GATHER SQL10_AWR_BUFFER_GET.SQL'
spool sql10_awr_buffer_get.sql
select '@sql10_awr.sql ' || e.snap_id || ' ' || e.end_time || ' ' || rnum || ' ' ||
  ...
  from (select d.snap_id,
          ...
          from (select c.snap_id,
                       ...
                       a.sql_id,
                       sum(a.BUFFER_GETS_DELTA) buffer_get
 ...
 where rnum < &&3;
spool off;
host grep '^@sql10_awr.sql' sql10_awr_buffer_get.sql >sql10_awr_buffer_gets.sql
@sql10_awr_buffer_gets.sql
host rm -rf sql10_awr_buffer_get.sql sql10_awr_buffer_gets.sql
