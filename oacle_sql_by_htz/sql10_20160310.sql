...
col EXECUTIONS              heading "EXEC"               for 99999999
col CPU_PRE_EXEC            heading "CPU(MS)|PRE EXEC"       for 999,999,999
col ELA_PRE_EXEC            heading "ELA(MS)|PRE EXEC"       for 999,999,999
...
col APP_WAIT_PRE            heading "APPLI(MS)|PER EXEC" for 999,999,999
col CON_WAIT_PER            heading "CONCUR(MS)|PER EXEC" for 999,999,999
col CLU_WAIT_PER            heading "CLUSTER(MS)|PER EXEC" FOR 999,999,999
...
--select * from table(dbms_xplan.display_cursor('&&sqlid',1,'advanced allstats last'));
--select * from table(dbms_xplan.display_cursor('&&sqlid',2,'advanced allstats last'));
--select * from table(dbms_xplan.display_cursor('&&sqlid',3,'advanced allstats last'));
--select * from table(dbms_xplan.display_cursor('&&sqlid',0));
--select * from table(dbms_xplan.display_cursor('&&sqlid',1));
--select * from table(dbms_xplan.display_cursor('&&sqlid',2));
--select * from table(dbms_xplan.display_cursor('&&sqlid',3));
--@tmp.sql
select t.*
  from v$sql s,
...
-- select distinct parsing_schema_name from v$sql where sql_id='&&sqlid';
