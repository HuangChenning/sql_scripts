...
###################################################################################
##                                                                               ##
## Author : HongyeDBA @ Enmotech (internal-only)                                 ##
##                                                                               ##
##-------------------------------------------------------------------------------##
##                +d:                                                            ##
##                `hds`                   .`                                     ##
...
##---------|------------------|--------------------------------------------------##
##     1.0 | 2014-05-22 13:50 | First Edition, (internal only)                   ##
##     2.0 | 2014-11-26 12:41 | Add Pre-Check for Duplicate Objects              ##
##         |                  | Add Filter Control Variables                     ##
##         |                  | Unpack with Parallel                             ##
##         |                  | Remove OS Staging File after Completed           ##
##         |                  | no Staging Table Created when Split and Unpack   ##
##         |                  |                                                  ##
###################################################################################
use strict;
use warnings;
...
# SQLSet tablename
our $sqlsetTabName = 'SQLSET_TAB_20150123';
# SQLSet owner name
our $sqlsetOwner = 'SPA';
# SQLSet owner password
our $sqlsetOwnerPass = 'spa';
# SQLSet name
our $sqlsetName = 'SQLSET_20150123';
# SPA task name prefix
our $spaTaskPrefix = 'SPA_TASK_20150123';
# SQL  Remove Filter
our $sqlRemoveFilter = "PARSING_SCHEMA_NAME IN ('SPA', 'DSG', 'IGNITE_M') OR PARSING_SCHEMA_NAME LIKE 'CD_%'";
# sqlFilterOK
our %sqlFilterOK = ('SYSTEM_SQL', 'YES', 'NON_EXECUTED', 'YES', 'CUST_FILTER', 'YES', 'NO_BIND', 'YES');
# SPA parallel
our $execParallels = 8;
# SQLSet version
our $sqlsetver = '11.2';
...
sub readDB{
    ...
    # Return First Line of the Result
    $errCNT = `head -1 $resultFile`;
    chomp $errCNT;
    return $errCNT;
}
...
sub preCheck{
  ...
  die 'Cannot do SQL in DB !!!' if(&executeSQL('select 1 from dual;'));

  # Check Duplicated SQLSET
  my $sqlsetCnt = &readDB("SELECT COUNT(*) FROM DBA_SQLSET WHERE NAME = '$sqlsetName' AND OWNER = '$sqlsetOwner';");
  for(my $x = 0; $x < $execParallels; $x ++){
    $sqlsetCnt += &readDB("SELECT COUNT(*) FROM DBA_SQLSET WHERE NAME = '${sqlsetName}_$x' AND OWNER = '$sqlsetOwner';");
  }
  die 'Duplicate SQLSET ['.$sqlsetName.'] Found ['.$sqlsetCnt.'] in DB !!!' if($sqlsetCnt);

  # Check Duplicated Task
  my $taskCnt = &readDB("SELECT COUNT(*) FROM DBA_ADVISOR_TASKS WHERE TASK_NAME = '$spaTaskPrefix' AND OWNER = '$sqlsetOwner';");
  for(my $x = 0; $x < $execParallels; $x ++){
    $taskCnt += &readDB("SELECT COUNT(*) FROM DBA_ADVISOR_TASKS WHERE TASK_NAME = '${spaTaskPrefix}_$x' AND OWNER = '$sqlsetOwner';");
  }
  die 'Duplicate Analysis Task ['.$spaTaskPrefix.'] Found ['.$taskCnt.'] in DB !!!' if($taskCnt);
}
...
sub preProcess{
  ...
  # Filter of System SQLs
  if(defined $sqlFilterOK{'SYSTEM_SQL'} eq 'YES'){
    ...
    printLOG('1. Keep System SQLs (Filter According PARSING_SCHEMA_NAME is Closed)');
  }

  # Filter of Non-Executed SQLs
  if(defined $sqlFilterOK{'NON_EXECUTED'} eq 'YES'){
    ...
    printLOG('2. Keep no Executed SQLs (Filter According EXECUTIONS is Closed)');
  }

  # Filter of Customer-Filter SQLs
  if(defined $sqlFilterOK{'CUST_FILTER'} eq 'YES'){
    printLOG('3. Remove SQL by Customer Filter');
    $sqlSTMT = 'DELETE FROM '.$sqlsetOwner.'.'.$sqlsetTabName." WHERE $sqlRemoveFilter;";
    executeSQL($sqlSTMT);
  }
  else{
    printLOG('3. Keep SQL by Customer Filter (Filter is Closed)');
  }

  # Filter of No-Bind SQLs
  if(defined $sqlFilterOK{'NO_BIND'} eq 'YES'){
    ...
    printLOG('4. Keep SQLs with no Bind (Filter According FORCE_MATCHING_SIGNATURE is Closed)');
  }
}
...
sub splitSQLSet{
  ...
  if ( $sqlsetver == "10.2") {
      ...
      BEGIN
        FOR X IN (SELECT SQL_ID FROM $sqlsetOwner.$sqlsetTabName ORDER BY ELAPSED_TIME ) LOOP
          UPDATE $sqlsetOwner.$sqlsetTabName
             SET NAME   = '${sqlsetName}_'||L_CURR_TABLE_TIPS
           WHERE SQL_ID = X.SQL_ID;
          L_CURR_TABLE_TIPS := MOD(L_CURR_TABLE_TIPS + 1, $execParallels);
        END LOOP;
      END;
      /";
  } elsif ($sqlsetver == "11.2") {
      my $maxID = &readDB("SELECT MAX(ID) FROM $sqlsetOwner.$sqlsetTabName;");
      $sqlSTMT = "DECLARE
        L_CURR_TABLE_TIPS   NUMBER   :=0;
      BEGIN
        FOR X IN (SELECT SQL_ID,MAX(SQL_SEQ) SQL_SEQ FROM $sqlsetOwner.$sqlsetTabName GROUP BY SQL_ID ORDER BY AVG(ELAPSED_TIME/EXECUTIONS)) LOOP
          UPDATE $sqlsetOwner.$sqlsetTabName
             SET NAME   = '${sqlsetName}_'||L_CURR_TABLE_TIPS
           WHERE SQL_ID = X.SQL_ID;
           DELETE FROM $sqlsetOwner.$sqlsetTabName
            WHERE SQL_ID=X.SQL_ID
              AND SQL_SEQ<>X.SQL_SEQ;
          L_CURR_TABLE_TIPS := MOD(L_CURR_TABLE_TIPS + 1, $execParallels);
        END LOOP;
      END;
      /";
...
sub unpackSQLSet{
  ...
  printLOG('Unpack SQL Set from Staging Table', 'Begin');

  printLOG('1. Unpack SQL Set');
  for (my $i = 0; $i < $execParallels; $i++){
    open EXECSCRIPT, '>', "./unpack_SPA_$i.sh";
    print EXECSCRIPT "echo true > unpack_SPA_wrok_$i.log\n";
    print EXECSCRIPT "sqlplus $sqlsetOwner/$sqlsetOwnerPass <<EOF\n";
    print EXECSCRIPT "EXEC DBMS_SQLTUNE.UNPACK_STGTAB_SQLSET ( -\n";
    print EXECSCRIPT "                  SQLSET_NAME          => '${sqlsetName}_$i', -\n";
    print EXECSCRIPT "                  SQLSET_OWNER         => '$sqlsetOwner', -\n";
    print EXECSCRIPT "                  REPLACE              => TRUE, -\n";
    print EXECSCRIPT "                  STAGING_TABLE_NAME   => '$sqlsetTabName', -\n";
    print EXECSCRIPT "                  STAGING_SCHEMA_OWNER => '$sqlsetOwner');\n";
    print EXECSCRIPT "EXIT\n";
    print EXECSCRIPT "EOF\n";
    print EXECSCRIPT "echo false > unpack_SPA_wrok_$i.log\n";
    close EXECSCRIPT;
    chmod 0755, "unpack_SPA_$i.sh";
    `nohup ./unpack_SPA_$i.sh 1>unpack_SPA_$i.log 2>&1 &`;
  }

  # Wait All Process Completed
  while (`ps -ef | grep unpack_SP[A] |grep -v grep| wc -l` > 0){sleep 1;}
#  while (`cat unpack_SPA_wrok_* | grep false| wc -l` > 0){sleep 1;}
  ...
  die 'SQLSET Count ['.$sqlsetCnt.'] Doesnot Match Parallel Settings ['.$execParallels.']' if($sqlsetCnt != $execParallels);

  printLOG('2. Revert "Name" Field in SQL Set Table');
  executeSQL("UPDATE $sqlsetOwner.$sqlsetTabName SET NAME = '$sqlsetName';");

  printLOG('3. Remove Unpack Staging File Created on OS');
  for (my $i = 0; $i < $execParallels; $i++){
    unlink "./unpack_SPA_$i.sh" if (-f "./unpack_SPA_$i.sh");
    unlink "./unpack_SPA_$i.log" if (-f "./unpack_SPA_$i.log");
  }

  printLOG('Unpack SQL Set from Staging Table', 'OK');
}
...
sub executeTrail{
  ...
  for (my $i = 0; $i < $execParallels; $i++){
    ...
    print EXECSCRIPT "echo true > exec_SPA_wrok_$i.log\n";
    print EXECSCRIPT "sqlplus $sqlsetOwner/$sqlsetOwnerPass <<EOF\n";
    ...
    print EXECSCRIPT "EXIT\n";
    print EXECSCRIPT "EOF\n";
    print EXECSCRIPT "echo false > exec_SPA_wrok_$i.log\n";
    close EXECSCRIPT;
    chmod 0755, "exec_SPA_$i.sh";
    my $bgTask = `nohup ./exec_SPA_$i.sh 1>exec_SPA_$i.log 2>&1 &`;
  }
  my $testExec = `ps -ef | grep exec_SP[A] |grep -v grep| wc -l`;
#  my $testExec = `cat exec_SPA_wrok* | grep fase | wc -l`;
  chomp($testExec);
  while ($testExec > 0){
    printLOG('   Execution Processes Does not Complete, Current Count is : ['.$testExec.']');
    sleep 60;
    $testExec = `ps -ef | grep exec_SP[A]| grep -v grep | wc -l`;
    chomp($testExec);
  }

  printLOG('2. Remove Execution Staging File Created on OS');
  for (my $i = 0; $i < $execParallels; $i++){
    unlink "./exec_SPA_$i.sh" if (-f "./exec_SPA_$i.sh");
    unlink "./exec_SPA_$i.log" if (-f "./exec_SPA_$i.log");
  }
...
# 3. Unpack SQL Set from Staging Table
unpackSQLSet;
