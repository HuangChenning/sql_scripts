REM
REM This healthcheck script is for use on Oracle12c (12.1.0.1) databases only.  
REM 
REM
...
declare
  ...
  streams_pool_size varchar2(512);
     ...
     cursor v1_capture is select capname_knstcap, substr(capname_knstcap,9,8) extract_name from x$knstcap x , cdb_capture c where bitand(x.flags_knstcap,64) <> 64 and  x.capname_knstcap=c.capture_name and c.purpose = 'GoldenGate Capture' and bitand(flags_knstcap,64) <>64;
...
select * from CDB_GOLDENGATE_SUPPORT_MODE where support_mode = 'NONE' order by con_id,owner,object_name;
...
col transaction_rate format a40 heading 'Transaction Rate| Txn per second'
col message_rate format a40 heading 'Message Apply Rate'
